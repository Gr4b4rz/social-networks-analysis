...
def plot_degree_dist(G):
    degree_sequence = sorted(
        [d for n, d in G.degree().items()], reverse=True)  # degree sequence
...
def calculate_avg_path_len(G, sampling):
    ...
    len_sum = 0
    while i < sampling:
        len_sum += nx.shortest_path_length(G,
                                           choice(G.nodes()), choice(G.nodes()))
        i += 1
...
def main():
    ...
    print("Srednia dlugosc sciezki dla proby 100: {}".format(
        calculate_avg_path_len(G, 100)))
    print("Srednia dlugosc sciezki dla proby 1000: {}".format(
        calculate_avg_path_len(G, 1000)))
    print("Srednia dlugosc sciezki dla proby 10000: {}".format(
        calculate_avg_path_len(G, 10000)))
